...
@role('admin')
<div class="card">
  <h5 class="card-header">Listado Postulante</h5>
  <!--Search Form -->
  <div class="card-body">
            ...
            <td>
                ...
                <form action="{{ route('admin.estudiantes.update',$estudiante->id) }}" method="POST">
                    ...
                    <select class="form-select" name="estado" onchange="this.form.submit()">
                        <option value="APROBADO" {{ $estudiante->estado == 'APROBADO' ? 'selected' : '' }}>Aprobado</option>
                        <option value="PENDIENTE" {{ $estudiante->estado == 'PENDIENTE' ? 'selected' : '' }}>Pendiente</option>
                        <option value="RECHAZADO" {{ $estudiante->estado == 'RECHAZADO' ? 'selected' : '' }}>Rechazado</option>
                    </select>
                </form>
            </td>
        </tr>
